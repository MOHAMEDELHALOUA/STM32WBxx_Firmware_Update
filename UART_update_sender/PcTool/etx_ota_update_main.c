...
compile with the command: gcc etx_ota_update_main.c RS232\rs232.c -IRS232 -Wall
-Wextra -o2 -o etx_ota_app
...
#define ETX_OTA_MAX_BLOCK_SIZE (4096)
#define ETX_OTA_MAX_FW_SIZE (ETX_OTA_MAX_BLOCK_SIZE * 32)
...
int main(int argc, char *argv[]) {
  ...
  do {
    if (argc <= 2) {
      printf(
          "Please feed the tty PORT number and the Application Image....!!!\n");
      printf("Example: .\\etx_ota_app.exe 8 "
             "..\\..\\Application\\Debug\\Blinky.bin");
      ex = -1;
      break;
    }
    ...
    for (uint32_t i = 0; i < app_size;) {
      ...
      if (i == app_size) {
        block_counter =
            (i + ETX_OTA_MAX_BLOCK_SIZE - 1) / ETX_OTA_MAX_BLOCK_SIZE;

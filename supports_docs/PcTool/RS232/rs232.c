...
struct termios new_port_settings, old_port_settings[RS232_PORTNR];
...
const char *comports[RS232_PORTNR] = {
    ...
    "/dev/ttyUSB4", "/dev/ttyUSB5", "/dev/ttyAMA0", "/dev/ttyAMA1",
    "/dev/ttyACM0", "/dev/ttyACM1", "/dev/rfcomm0", "/dev/rfcomm1",
    "/dev/ircomm0", "/dev/ircomm1", "/dev/cuau0",   "/dev/cuau1",
    "/dev/cuau2",   "/dev/cuau3",   "/dev/cuaU0",   "/dev/cuaU1",
    "/dev/cuaU2",   "/dev/cuaU3"};
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode,
  ...
  */
  ...
  if (error == -1) {
    close(Cport[comport_number]);
    flock(Cport[comport_number],
          LOCK_UN); /* free the port so that others can use it. */
    perror("unable to read portsettings ");
    return (1);
  }
  memset(&new_port_settings, 0,
         sizeof(new_port_settings)); /* clear the new struct */
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode,
  ...
  */
  ...
  Cport[comport_number] = CreateFileA(
      comports[comport_number], GENERIC_READ | GENERIC_WRITE, 0, /* no share  */
      NULL,             /* no security */
...
int RS232_PollComport(int comport_number, unsigned char *buf, int size) {
  ...
  if (!ReadFile(Cport[comport_number], buf, size, (LPDWORD)((void *)&n),
                NULL)) {
    return -1;
  }
...
int RS232_SendByte(int comport_number, unsigned char byte) {
  ...
  if (!WriteFile(Cport[comport_number], &byte, 1, (LPDWORD)((void *)&n),
                 NULL)) {
    return (1);
  }
...
void RS232_cputs(int comport_number,
                 const char *text) /* sends a string to serial port */
{
  while (*text != 0)
